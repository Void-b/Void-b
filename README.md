# Hi, I’m James Francis ( Void-bit )

A **Flutter Developer** passionate about building reliable, elegant, and production-ready apps.
I’ve spent years freelancing — solving real-world problems across startups and individuals — and I’m now focused on bringing that experience into a **corporate team environment** where collaboration, clean architecture, and delivery matter.

---

### Core Skills

* **Flutter & Dart** (mobile + web + desktop)
* **Backend Integration:** Firebase, Supabase, Node.js
* **Architecture:** Clean / Modular / Scalable
* **Payments:** Paystack · Stripe · Card Reuse Flows
* **Databases:** Firestore · MongoDB · PostgreSQL
* **Dev Tools:** Git · CI/CD · Agile Workflows

---

### What I Build

| Project          | Description                                                    |
| ---------------- | -------------------------------------------------------------- |
| **TeamFlow**     | Task + chat platform for internal teams with role-based access |
| **MultiStore**   | Multi-vendor e-commerce app with Paystack integration          |
| **InsightBoard** | Web analytics dashboard for business metrics                   |
| **SmartWriter**  | AI-assisted content generator for creators                     |

> See all projects on [GitHub Repositories](https://github.com/Void-b?tab=repositories)

---

### GitHub Stats

[![GitHub Streak](https://streak-stats.demolab.com?user=Void-b\&theme=dark\&hide_border=true)](https://git.io/streak-stats)

![Stats](https://github-readme-stats.vercel.app/api?username=Void-b\&show_icons=true\&theme=dark\&hide_border=true)
![Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Void-b\&layout=compact\&theme=dark\&hide_border=true)

---

### Connect

📧 [fr1ncis.j1mes54@gmail.com](mailto:fr1ncis.j1mes54@gmail.com)
💼 [LinkedIn](https://www.linkedin.com/in/francis-james500/) | 🐦 [Twitter / X](https://x.com/FJekems) | 🌍 [Portfolio](https://void-b.vercel.app/)

> *“Creating impact through purposeful code.”*
> — Void-bit
